<script setup>
import { ref } from 'vue';
import { api } from '../utils';
import {
  NAvatar,
  NButton,
  NList,
  NListItem,
  NSpace,
  NStatistic,
  NNumberAnimation,
} from 'naive-ui';

var lastUpdate = ref('Loading...');
var csbzbsyyList = [];

api.get('/rank/csbzbsyy').then((response) => {
  const { data } = response;
  lastUpdate.value = new Date(data.cached_at * 1000).toLocaleString();
  csbzbsyyList = data.content;
});
</script>

<template>
  <NList bordered hoverable clickable style="min-width: 500px">
    <template #header>
      <h2>30 日成事不足败事有余榜</h2>
    </template>
    <template #footer> 上次更新：{{ lastUpdate }} </template>
    <NListItem v-for="(csbzbsyy, i) in csbzbsyyList" :key="i">
      <template #prefix> #{{ i + 1 }} </template>
      <RouterLink style="display: block" :to="'/historyFeed/' + csbzbsyy.uid">
        <NSpace style="display: flex; align-items: center">
          <NAvatar
            style="display: inline-block"
            round
            size="small"
            :src="
              'https://cdn.luogu.com.cn/upload/usericon/' +
              csbzbsyy.uid +
              '.png'
            "
          />
          <NButton text>{{ csbzbsyy.name }}</NButton>
        </NSpace>
      </RouterLink>
      <template #suffix>
        <NStatistic tabular-nums>
          <NNumberAnimation v-if="i < 10" :from="0" :to="csbzbsyy.count" />
          <span v-else>{{ csbzbsyy.count }}</span>
        </NStatistic>
      </template>
    </NListItem>
  </NList>
</template>
