...
api.get('/rank/tietie').then((response) => {
  const { data } = response;
    ...
    <NListItem v-for="(tietie, i) in tietieList" :key="i">
      ...
      <RouterLink style="display: block" :to="'/historyFeed/' + tietie.uid">
        <NSpace style="display: flex; align-items: center">
          <NAvatar
            ...
            size="small"
            :src="
              'https://cdn.luogu.com.cn/upload/usericon/' + tietie.uid + '.png'
            "
          />
      ...
      <template #suffix>
        <NStatistic tabular-nums>
          <span v-if="i < 10" style="color: rgb(255, 141, 160)"
            ><NNumberAnimation :from="0" :to="tietie.count"
          /></span>
